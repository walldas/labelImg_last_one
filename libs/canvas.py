...
class Canvas(QWidget):
    ...
    def getRotatedShape(self,shape,angle):
        return [Shape.rotatePoint(self, shape.centerPoint,point,angle) for point in shape.points]
    ...
    def angleBetween3Points(self,p0,c,p1):
        p0c = math.sqrt(math.pow(c.x()-p0.x(),2)+ math.pow(c.y()-p0.y(),2)) # p0->c (b)
        p1c = math.sqrt(math.pow(c.x()-p1.x(),2)+ math.pow(c.y()-p1.y(),2)) # p1->c (a)
        p0p1 = math.sqrt(math.pow(p1.x()-p0.x(),2)+ math.pow(p1.y()-p0.y(),2)) # p0->p1 (c)
        ratio=math.acos((p1c*p1c+p0c*p0c-p0p1*p0p1)/(2*p1c*p0c))
    ...
    def paintEvent(self, event):
        ...
        # Paint rect
        if self.current is not None and len(self.line) == 2 and self.draw3DMode==False:
            leftTop = self.line[0]
